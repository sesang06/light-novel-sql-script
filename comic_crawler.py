...
def is_comic_in_database(item_id):
    sql = "SELECT `id` FROM comic WHERE `aladin_id`=%s;"
...
def insert_comic_series(series_object):
    ...
    if result is None:
        ...
        connection.commit()
def update_comic_series_id(series_object):
...
def get_comic_info(data):
    sql = "SELECT * FROM comic WHERE `aladin_id`=%s;"
    cursor.execute(sql, data['aladin_id'])
    result = cursor.fetchone()
    return result
def update_series_info(data):
    if data['series_aladin_id'] == 0 and data['adult'] == 0:
        ...
        if series_id != 0:
            ...
            insert_comic_series(series_object)
            update_comic_series_id(series_object)

def update_comic_description(description_object):
    sql = """
    UPDATE comic
    SET 
    updated_at = now(),
...
def update_description_info(data):
    if data['publisher_description'] == "" or data['index_description'] == "":
        ...
        update_comic_description(description_object)
time = get_now()
print('b')
for i in range(1,21):
    print('c')
